...
Replace runtime Bun shell template-tag usage in `packages/papecode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/papecode/src` is removed except:

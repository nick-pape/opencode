...
- `packages/core` - all papecode services, Effect-first source of truth
- `packages/server` - papecode server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full papecode server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/papecode`.
...
- those slices are mounted into the Hono server behind `PAPECODE_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `papecode` package split is earlier in the migration.
...
- the current server host still lives in `packages/papecode`
...
- if `packages/server` imports services from `packages/papecode` while `packages/papecode` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/papecode` as the temporary Hono host
...
- `packages/server` must not import from `packages/papecode`

Allowed in phase 1:

- `packages/papecode` imports `packages/server`
...
- `packages/papecode` shrinks or disappears as package responsibilities are fully split
...
- `packages/papecode` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/papecode`
...
Keep in `packages/papecode` for now:
...
- host mounting from `packages/papecode`
...
- no imports from `packages/papecode` inside `packages/server`
...
- `packages/server` must still not import from `packages/papecode`
...
### PR 4. Mount `packages/server` question from `packages/papecode`

Scope:

- replace local experimental question route wiring in `packages/papecode`
...
- mount it in parallel from `packages/papecode`
...
- move adapter selection and server startup out of `packages/papecode`

Rules:

- do not start this while `packages/server` still depends on `packages/papecode`
...
- host logic no longer lives in `packages/papecode`
...
3. the route is mounted from the current host in `packages/papecode`
...
- `packages/server` imports services or runtime from `packages/papecode`
- `packages/papecode` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/papecode` imports
...
- [ ] mount `question` from `packages/papecode`
...
2. keep `packages/papecode` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/papecode`, stop and restructure the boundary first.

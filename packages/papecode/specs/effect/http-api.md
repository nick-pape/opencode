...
- `PAPECODE_EXPERIMENTAL_HTTPAPI` gates the bridge. Default behavior still uses Hono.
...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-papecode-directory`.
...
- Cover `directory` and `x-papecode-directory` instance selection.
...
- [x] Bridge selected `HttpApi` routes into Hono behind `PAPECODE_EXPERIMENTAL_HTTPAPI`.

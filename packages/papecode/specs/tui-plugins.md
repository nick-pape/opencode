...
- Author package entrypoint is `@papecode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://papecode.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/papecode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@papecode-ai/plugin/tui`.
- `@papecode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@papecode-ai/plugin/tui"
...
- The value is a semver range checked against the running PapeCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.papecode` is absent, no check is performed (backward compatible).
...
- `papecode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `papecode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.papecode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.papecode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.papecode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.papecode` dir or `PAPECODE_CONFIG_DIR`, PapeCode installs `@papecode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@papecode-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.papecode/themes` area near the plugin config source.
...
- `--pure` / `PAPECODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.papecode/plugins/tui-smoke.tsx`
- Local vim plugin: `.papecode/plugins/tui-vim.tsx`
- Local smoke config: `.papecode/tui.json`
- Local smoke theme: `.papecode/plugins/smoke-theme.json`

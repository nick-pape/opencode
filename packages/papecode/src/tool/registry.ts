...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@papecode-ai/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@papecode/ToolRegistry") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.PAPECODE_CLIENT) || Flag.PAPECODE_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            ...(Flag.PAPECODE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            ...(Flag.PAPECODE_EXPERIMENTAL_PLAN_MODE && Flag.PAPECODE_CLIENT === "cli" ? [tool.plan] : []),
          ],
          task: tool.task,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === CodeSearchTool.id || tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID.papecode || Flag.PAPECODE_ENABLE_EXA
        }

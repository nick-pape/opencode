...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal papecode sessions
...
papecode acp --cwd /path/to/project
```
...
```bash
PAPECODE_ENABLE_QUESTION_TOOL=1 papecode acp
```
...
- **Terminal Integration**: Full terminal support via papecode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | papecode acp
```
...
ACP sessions map cleanly to papecode's internal session model:

...
import type { AssistantMessage, Event, PapecodeClient, SessionMessageResponse, ToolPart } from "@papecode-ai/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: PapecodeClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `papecode auth login` in the terminal",
      name: "Login with papecode",
      id: "papecode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // PapeCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const papecodeProvider = providers.find((p) => p.id === "papecode")
  if (papecodeProvider) {
    if (papecodeProvider.models["big-pickle"]) {
      return { providerID: ProviderID.papecode, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(papecodeProvider.models))
    if (best) {
      return {

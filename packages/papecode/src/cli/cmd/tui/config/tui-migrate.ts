...
const TUI_SCHEMA_URL = "https://papecode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from papecode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const papecode = await papecodeFiles(input)
  for (const file of papecode) {
...
async function papecodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "papecode"),
    ...(await Filesystem.findUp(["papecode.json", "papecode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "papecode"))
  }
  if (Flag.PAPECODE_CONFIG) files.push(Flag.PAPECODE_CONFIG)

...
import papecode from "./theme/papecode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@papecode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "papecode")
        draft.active = typeof active === "string" ? active : "papecode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.papecode, store.mode)
    })

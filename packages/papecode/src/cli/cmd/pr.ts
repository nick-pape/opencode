...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run papecode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for papecode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found papecode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["papecode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting papecode...")
        UI.println()

        const papecodeArgs = sessionId ? ["-s", sessionId] : []
        const papecodeProcess = Process.spawn(["papecode", ...papecodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await papecodeProcess.exited
        if (code !== 0) throw new Error(`papecode exited with code ${code}`)
      },
    })
  },
})

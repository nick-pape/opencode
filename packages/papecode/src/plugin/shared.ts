...
export const DEPRECATED_PLUGIN_PACKAGES = ["papecode-openai-codex-auth", "papecode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, papecodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(papecodeVersion) || semver.major(papecodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.papecode
  if (typeof range !== "string") return
  if (!semver.satisfies(papecodeVersion, range)) {
    throw new Error(`Plugin requires papecode ${range} but running ${papecodeVersion}`)
  }
}

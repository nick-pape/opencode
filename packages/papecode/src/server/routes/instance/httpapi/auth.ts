...
export class Authorization extends HttpApiMiddleware.Service<Authorization>()(
  "@papecode/ExperimentalHttpApiAuthorization",
  {
    error: Unauthorized,
    security: {
...
function validateCredential<A, E, R>(
  ...
  return Effect.gen(function* () {
    if (!Flag.PAPECODE_SERVER_PASSWORD) return yield* effect

    if (credential.username !== (Flag.PAPECODE_SERVER_USERNAME ?? "papecode")) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    if (Redacted.value(credential.password) !== Flag.PAPECODE_SERVER_PASSWORD) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    return yield* effect
  })

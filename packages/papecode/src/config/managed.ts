...
const MANAGED_PLIST_DOMAIN = "ai.papecode.managed"

// Keys injected by macOS/MDM into the managed plist that are not PapeCode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/papecode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "papecode")
    default:
      return "/etc/papecode"
  }
}
...
export function managedConfigDir() {
  return process.env.PAPECODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}

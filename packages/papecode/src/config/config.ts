...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in papecode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for papecode serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://papecode.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://papecode.ai/docs/agents" }),
...
export class Service extends Context.Service<Service, Interface>()("@papecode/Config") {}

function globalConfigFile() {
  const candidates = ["papecode.jsonc", "papecode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://papecode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "papecode.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "papecode.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://papecode.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "PAPECODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/papecode` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/papecode`))
            if (!response.ok) {
              throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
            }
            const wellknown = (yield* Effect.promise(() => response.json())) as { config?: Record<string, unknown> }
            const remoteConfig = wellknown.config ?? {}
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://papecode.ai/config.json"
            const source = `${url}/.well-known/papecode`
        ...
        if (Flag.PAPECODE_CONFIG) {
          yield* merge(Flag.PAPECODE_CONFIG, yield* loadFile(Flag.PAPECODE_CONFIG))
          log.debug("loaded custom config", { path: Flag.PAPECODE_CONFIG })
        }

        if (!Flag.PAPECODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("papecode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.PAPECODE_CONFIG_DIR) {
          log.debug("loading config from PAPECODE_CONFIG_DIR", { path: Flag.PAPECODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".papecode") || dir === Flag.PAPECODE_CONFIG_DIR) {
            for (const file of ["papecode.json", "papecode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.papecode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.PAPECODE_CONFIG_CONTENT) {
          const source = "PAPECODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.PAPECODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from PAPECODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["PAPECODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("PAPECODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.PAPECODE_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.PAPECODE_PERMISSION))
        }
        ...
        if (Flag.PAPECODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.PAPECODE_DISABLE_PRUNE) {

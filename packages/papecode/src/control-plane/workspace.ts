...
import { Slug } from "@papecode-ai/shared/util/slug"
...
export const create = fn(CreateInput.zod, async (input) => {
  ...
  const env = {
    PAPECODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    PAPECODE_WORKSPACE_ID: config.id,
    PAPECODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.PAPECODE_EXPERIMENTAL_WORKSPACES) return

...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via npm view", async () => {
      ...
      expect(calls).toContainEqual(["npm", "view", `papecode-ai@${InstallationChannel}`, "version", "--json"])
    })
    ...
    test("reads npm versions via bun pm view", async () => {
      ...
      expect(calls).toContainEqual(["bun", "pm", "view", `papecode-ai@${InstallationChannel}`, "version", "--json"])
    })
    ...
    test("reads npm versions via pnpm view", async () => {
      ...
      expect(calls).toContainEqual(["pnpm", "view", `papecode-ai@${InstallationChannel}`, "version", "--json"])
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("anomalyco/tap/papecode")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("papecode")) return "papecode"
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("anomalyco/tap/papecode") && args.includes("--formula")) return "papecode"

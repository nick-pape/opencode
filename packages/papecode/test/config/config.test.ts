...
const managedConfigDir = process.env.PAPECODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "papecode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "papecode.json") {
...
test("ignores legacy tui keys in papecode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://papecode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "papecode.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["PAPECODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { papecode: { options: { apiKey: "{env:PAPECODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["papecode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["PAPECODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["PAPECODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "papecode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .papecode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const papecodeDir = path.join(dir, ".papecode")
      await fs.mkdir(papecodeDir, { recursive: true })
      const agentDir = path.join(papecodeDir, "agent")
...
test("loads agents from .papecode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const papecodeDir = path.join(dir, ".papecode")
      await fs.mkdir(papecodeDir, { recursive: true })

      const agentsDir = path.join(papecodeDir, "agents")
...
test("loads commands from .papecode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const papecodeDir = path.join(dir, ".papecode")
      await fs.mkdir(papecodeDir, { recursive: true })

      const commandDir = path.join(papecodeDir, "command")
...
test("loads commands from .papecode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const papecodeDir = path.join(dir, ".papecode")
      await fs.mkdir(papecodeDir, { recursive: true })

      const commandsDir = path.join(papecodeDir, "commands")
...
test("does not try to install dependencies in read-only PAPECODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.PAPECODE_CONFIG_DIR
  process.env.PAPECODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.PAPECODE_CONFIG_DIR
    else process.env.PAPECODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable PAPECODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "papecode.json"),
        JSON.stringify({ $schema: "https://papecode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .papecode config
      const projectDir = path.join(dir, "project")
      const papecodeDir = path.join(projectDir, ".papecode")
      await fs.mkdir(papecodeDir, { recursive: true })
      ...
      // Local .papecode config with different plugins
      await Filesystem.write(
        path.join(papecodeDir, "papecode.json"),
        JSON.stringify({
          $schema: "https://papecode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .papecode config with some overlapping plugins
      await Filesystem.write(
        path.join(papecodeDir, "papecode.json"),
        JSON.stringify({
          $schema: "https://papecode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".papecode")
...
// Note: preload.ts sets PAPECODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .papecode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .papecode directory config enables it
      const papecodeDir = path.join(dir, ".papecode")
      await fs.mkdir(papecodeDir, { recursive: true })
      await Filesystem.write(
        path.join(papecodeDir, "papecode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/papecode")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/papecode")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "papecode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-papecode@2.4.3", file)).toBe("oh-my-papecode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-papecode@2.4.3", "file:///project/.papecode/plugin/oh-my-papecode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.papecode/plugin/demo.ts", "file:///project/.papecode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.papecode/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const papecodeDir = path.join(projectDir, ".papecode")
        const pluginDir = path.join(papecodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("PAPECODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["PAPECODE_DISABLE_PROJECT_CONFIG"]
    process.env["PAPECODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["PAPECODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["PAPECODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .papecode/ directories when flag is set", async () => {
    const originalEnv = process.env["PAPECODE_DISABLE_PROJECT_CONFIG"]
    process.env["PAPECODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .papecode directory with a command
          const papecodeDir = path.join(dir, ".papecode", "command")
          await fs.mkdir(papecodeDir, { recursive: true })
          await Filesystem.write(path.join(papecodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .papecode should NOT be in directories list
          const hasProjectPapecode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectPapecode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["PAPECODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PAPECODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["PAPECODE_CONFIG_DIR"]
      process.env["PAPECODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["PAPECODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["PAPECODE_CONFIG_DIR"]
      } else {
        process.env["PAPECODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("PAPECODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["PAPECODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["PAPECODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["PAPECODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["PAPECODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from PAPECODE_CONFIG_DIR, not project
...
describe("PAPECODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in PAPECODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PAPECODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["PAPECODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://papecode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["PAPECODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["PAPECODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in PAPECODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["PAPECODE_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "PapeCode Managed",
          PayloadIdentifier: "ai.papecode.managed.test",
          PayloadType: "ai.papecode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    Config.Info.zod,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://papecode.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://papecode.ai/config.json")
})

...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "papecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "papecode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "papecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "papecode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["papecode.json", "papecode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "papecode.json"),
        path.join(tmp.path, "papecode.jsonc"),
        path.join(project, "papecode.json"),
        path.join(project, "papecode.jsonc"),
      ])
    })
  })

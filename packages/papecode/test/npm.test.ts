...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@papecode/acme")).toBe("@papecode/acme")
    expect(Npm.sanitize("@papecode/acme@1.0.0")).toBe("@papecode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/papecode/acme.git"
    const expected = win ? "acme@git+https_//github.com/papecode/acme.git" : spec

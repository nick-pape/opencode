...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("papecode")]
        ...
        JSON.stringify({
          ...
          provider: {
            "cloudflare-ai-gateway": {
              options: {
                metadata: { invoked_by: "test", project: "papecode" },
              },
            },
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".papecode", "plugin")
...
test("papecode loader keeps paid models when config apiKey is present", async () => {
...
test("papecode loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "papecode.json"),

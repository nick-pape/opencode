...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under papecode.", () => {
    expect(paramToAttributeKey("name")).toBe("papecode.name")
    expect(paramToAttributeKey("slug")).toBe("papecode.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["papecode.sessionID"]).toBeUndefined()
    expect(attrs["papecode.messageID"]).toBeUndefined()
    expect(attrs["papecode.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under papecode. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["papecode.name"]).toBe("exa")

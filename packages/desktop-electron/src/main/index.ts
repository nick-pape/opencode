...
process.env.PAPECODE_DISABLE_EMBEDDED_WEB_UI = "true"
...
const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.papecode.desktop.dev"
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "PapeCode Dev")
...
import type { Server } from "virtual:papecode-server"
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("papecode://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    app.setAsDefaultProtocolClient("papecode")
    registerRendererProtocol()
    setDockIcon()
    setupAutoUpdater()
    await initialize()
...
async function initialize() {
  ...
  const loadingTask = (async () => {
    ...
    if (needsMigration) {
      const { Database, JsonMigration } = await import("virtual:papecode-server")
...
async function getSidecarPort() {
  const fromEnv = process.env.PAPECODE_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "papecode", "papecode.db"))
}

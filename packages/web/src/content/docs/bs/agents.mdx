...
Postoje dvije vrste agenata u PapeCode: primarni agenti i podagenti.
...
PapeCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
PapeCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

PapeCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `papecode.json`:
...
- Globalno: `~/.config/papecode/agents/`
- Po projektu: `.papecode/agents/`
...
Ako temperatura nije navedena, PapeCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu PapeCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj PapeCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [PapeCode Zen](/docs/zen), koristili biste `papecode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `papecode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/papecode).
:::

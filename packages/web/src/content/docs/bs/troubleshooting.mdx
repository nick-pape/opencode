...
Da biste otklonili probleme s PapeCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\papecode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `papecode --log-level DEBUG`.

---

## Pohrana

PapeCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/papecode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\papecode`
...
PapeCode Desktop pokreće lokalni PapeCode server (`papecode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `PapeCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/papecode/papecode.jsonc` (ili `~/.config/papecode/papecode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/papecode/papecode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
PapeCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/papecode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\papecode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga PapeCode mogao ponovo izgraditi.

1. Potpuno zatvorite PapeCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/papecode`
- **Linux**: obrišite `~/.cache/papecode` (ili pokrenite `rm -rf ~/.cache/papecode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\papecode`

3. Ponovo pokrenite PapeCode Desktop.

---

### Rješavanje problema sa vezom na serveru

PapeCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `papecode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `PAPECODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `PAPECODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, PapeCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa PapeCode karakteristikama.

---

### Obavještenja se ne prikazuju

PapeCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za PapeCode u postavkama vašeg OS-a, i
...
1. Zatvorite PapeCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka PapeCode Desktop aplikacije):

- `papecode.settings.dat` (URL zadanog servera za desktop)
- `papecode.global.dat` i `papecode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `papecode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\papecode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. PapeCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\papecode`

2. Ponovo pokrenite PapeCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti PapeCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
PapeCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

...
ove ili konfigurirajte svoje putem papecode config.
...
papecode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.papecode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `papecode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/papecode/modes/`
- Projekat: `.papecode/modes/`
...
Ako temperatura nije navedena, papecode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.papecode/modes/` za specifične načine rada ili `~/.config/papecode/modes/` za globalne načine:

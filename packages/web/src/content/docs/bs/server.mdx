---
title: Server
description: Komunicirajte s papecode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `papecode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji papecode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `papecode.local` |
...
Postavite `PAPECODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `papecode`, ili postavite `PAPECODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `papecode serve` i za `papecode web`.
...
Kada pokrenete `papecode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite papecode server za programsku interakciju sa papecode-om.
:::

Ova arhitektura omogucava papecode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `papecode serve` da startate standalone server. Ako je papecode TUI vec pokrenut, `papecode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste PapeCode [IDE](/docs/ide) pluginovi.
...
papecode server izlaže sljedece API-je.

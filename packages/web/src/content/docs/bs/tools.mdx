...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. PapeCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u PapeCode.
...
:::note
Ovaj alat je dostupan samo kada je `PAPECODE_EXPERIMENTAL_LSP_TOOL=true` (ili `PAPECODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz PapeCode provajdera ili kada je varijabla `PAPECODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju PapeCode:

---
title: Konfiguracija
description: Korištenje PapeCode JSON konfiguracije.
---

Možete konfigurirati PapeCode koristeći JSON konfiguracijski fajl.

---

## Format

PapeCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/papecode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/papecode/papecode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`PAPECODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`papecode.json` u projektu) - postavke specifične za projekat
5. **`.papecode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`PAPECODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.papecode` i `~/.config/papecode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/papecode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu PapeCode konfiguraciju u `~/.config/papecode/papecode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/papecode/tui.json`.
...
Dodajte `papecode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se PapeCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `PAPECODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `PAPECODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.papecode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.papecode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`papecode.ai/config.json`**](https://papecode.ai/config.json).

TUI konfiguracija koristi [**`papecode.ai/tui.json`**](https://papecode.ai/tui.json).
...
Koristite `PAPECODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `papecode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `papecode serve` i `papecode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš PapeCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `papecode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj PapeCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, PapeCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/papecode/agents/` ili `.papecode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, PapeCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`papecode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/papecode/commands/` ili `.papecode/commands/`. [Saznajte više ovdje](/docs/commands).
...
PapeCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, PapeCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju PapeCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.papecode/plugins/` ili `~/.config/papecode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite PapeCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

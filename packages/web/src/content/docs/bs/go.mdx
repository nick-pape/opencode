...
PapeCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
PapeCode Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u PapeCode-u. Pretplatite se na PapeCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili PapeCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa PapeCode-om.
:::
...
PapeCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

PapeCode Go radi kao bilo koji drugi provajder u PapeCode-u.

1. Prijavite se na **<a href={console}>PapeCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `PapeCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na PapeCode Go.
:::
...
PapeCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj PapeCode konfiguraciji
koristi format `papecode-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`papecode-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo PapeCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz PapeCode koristite i bilo kojeg drugog provajdera.

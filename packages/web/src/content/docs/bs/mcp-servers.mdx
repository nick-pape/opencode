...
Možete dodati vanjske alate u PapeCode koristeći _Model Context Protocol_, ili MCP. PapeCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [PapeCode Config](https://papecode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/papecode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
PapeCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, PapeCode će:
...
Ako server zahtijeva autentifikaciju, PapeCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `papecode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, PapeCode sigurno čuva tokene u `~/.local/share/papecode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u PapeCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz PapeCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete PapeCode sa vašim Sentry nalogom.

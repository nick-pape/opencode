...
U PapeCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, PapeCode koristi nasu `papecode` temu.
...
PapeCode dolazi sa vise ugradenih tema.
...
- Zele da PapeCode odgovara izgledu njihovog terminala
...
PapeCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/papecode/themes/*.json` ili `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.papecode/themes/*.json`
4. **Trenutni radni direktorij** - `./.papecode/themes/*.json`

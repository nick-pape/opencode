---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u PapeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PapeCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj PapeCode konfiguraciji.
...
u `~/.local/share/papecode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem PapeCode
config.
...
PapeCode Zen je lista modela koje je obezbedio PapeCode tim koji su bili
testirano i potvrđeno da dobro radi sa PapeCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa PapeCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite papecode i idite na [papecode.ai/auth](https://papecode.ai/auth).
...
Radi kao i svaki drugi provajder u PapeCode i potpuno je opcionalan za korištenje.

---

## PapeCode Go

PapeCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža PapeCode tim i koji su testirani i verificirani da dobro rade s PapeCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `PapeCode Go` i idite na [papecode.ai/auth](https://papecode.ai/zen).
...
Da biste koristili Amazon Bedrock s PapeCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete papecode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `papecode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u PapeCode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi papecode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju papecode konfiguraciju.
...
da pohrani token u papecode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
PapeCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate PapeCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`papecode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `papecode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s papecode:
...
Za korištenje Google Vertex AI s PapeCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće papecode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem papecodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) da automatski prijavite svaki PapeCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki PapeCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati papecode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati papecode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati papecode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za PapeCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/papecode) za detalje.
:::
...
Da biste koristili Ollama Cloud s PapeCode:
...
3. Kopirajte API ključ za korištenje u PapeCode.
...
6. **Važno**: Prije upotrebe modela oblaka u PapeCode, morate lokalno povući informacije o modelu:
...
PapeCode Zen je lista testiranih i verifikovanih modela koju je obezbedio PapeCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>PapeCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **PapeCode Zen**.

```txt
   /connect
```

3. Unesite svoj PapeCode API ključ.
...
Također možete dodati dodatne modele putem vaše papecode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše papecode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Papecodeom:
...
Također možete prilagoditi modele kroz svoju papecode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s papecode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `papecode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju PapeCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `papecode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite PapeCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj papecode konfiguraciji.

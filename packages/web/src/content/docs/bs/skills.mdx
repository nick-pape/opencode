...
Agent skills omogucavaju PapeCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
PapeCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.papecode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/papecode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, PapeCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.papecode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.papecode/skills/git-release/SKILL.md` ovako:
...
PapeCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `papecode.json`:
...
**Za ugradene agente** (u `papecode.json`):

---
title: Dodaci
description: Napišite vlastite dodatke za proširenje PapeCode.
---

Dodaci vam omogućavaju da proširite PapeCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja PapeCode.
...
- `.papecode/plugins/` - Dodaci na nivou projekta
- `~/.config/papecode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/papecode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/papecode/papecode.json`)
2. Konfiguracija projekta (`papecode.json`)
3. Globalni direktorij dodataka (`~/.config/papecode/plugins/`)
4. Direktorij dodataka projekta (`.papecode/plugins/`)
...
PapeCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Papecode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje PapeCode.
...
Spriječite papecode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u papecode:
...
Pomoćnik `tool` kreira prilagođeni alat koji papecode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za papecode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://papecode.ai/docs/sdk) za detalje.

---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć papecode.
---

Wtyczki umożliwiające wyłączenie papecode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania papecode.
...
- `.papecode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/papecode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/papecode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/papecode/papecode.json`)
2. Konfiguracja projektu (`papecode.json`)
3. Globalny katalog wtyczek (`~/.config/papecode/plugins/`)
4. Katalog wtyczek projektu (`.papecode/plugins/`)
...
papecode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z papecode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do papecode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej papecode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj papecode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do papecode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod papecode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://papecode.ai/docs/sdk).

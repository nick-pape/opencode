...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. PapeCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w PapeCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `PAPECODE_EXPERIMENTAL_LSP_TOOL=true` (lub `PAPECODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy PapeCode lub gdy zmienna środowiskowa `PAPECODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu PapeCode:

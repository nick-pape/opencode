...
Umiejętności agenta pozwalają papecode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
papecode przeszukuje te lokalizacje:

- Project config: `.papecode/skills/<name>/SKILL.md`
- Global config: `~/.config/papecode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu papecode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.papecode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.papecode/skills/git-release/SKILL.md` w ten sposób:
...
papecode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `papecode.json`:
...
**Dla agentów wbudowanych** (w `papecode.json`):

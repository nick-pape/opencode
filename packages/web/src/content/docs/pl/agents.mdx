...
W PapeCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
PapeCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
PapeCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

PapeCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `papecode.json`:
...
- Globalnie: `~/.config/papecode/agents/`
- W projekcie: `.papecode/agents/`
...
Jeśli nie zostanie skonfigurowana, PapeCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji PapeCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji PapeCode w `provider/model-id`. Na przykład, [PapeCode Zen](/docs/zen), użyj `papecode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `papecode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/papecode).
:::

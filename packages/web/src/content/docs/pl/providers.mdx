---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w papecode.
---

import config from "../../../../config.mjs"
export const console = config.console

papecode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji papecode.
...
w `~/.local/share/papecode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim papecode
config.
...
PapeCode Zen to lista modeli dostarczonych przez zespół papecode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z papecode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od PapeCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `PapeCode Zen` i przejdź do [papecode.ai/auth](https://papecode.ai/zen).
...
Działa jak każdy inny dostawca w papecode i jest całkowicie opcjonalny w użyciu.

---

## PapeCode Go

PapeCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół papecode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z papecode.

1. Uruchom polecenie `/connect` w TUI, wybierz `PapeCode Go` i przejdź do [papecode.ai/auth](https://papecode.ai/zen).
...
Aby używać Amazon Bedrock z papecode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania papecode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `papecode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w papecode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod papecode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji papecode.
...
papecode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować papecode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`papecode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth).

##### Konfiguracja

Customize through `papecode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem papecode:
...
Aby używać Google Vertex AI z papecode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania papecode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą papecode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session), aby automatycznie rejestrować każdą konwersację papecode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja papecode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować papecode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować papecode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować papecode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem papecode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/papecode).
:::
...
Aby korzystać z Ollama Cloud z papecode:
...
3. Skopiuj klucz API do użycia w papecode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w papecode musisz pobrać informacje o modelu lokalnie:
...
PapeCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół papecode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>PapeCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **PapeCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API papecode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji papecode.
...
5. Możesz je także dostosować za pomocą konfiguracji papecode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z papecode:
...
Możesz także dostosować modele za pomocą konfiguracji papecode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z papecode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `papecode.json` w katalogu projektu:
...
Pola `limit` pozwalają papecode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `papecode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację papecode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji papecode.

...
Możesz dodać dodatkowe informacje do papecode za pomocą _Model Context Protocol_, w skrócie MCP. papecode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [papecode Config](https://papecode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/papecode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
papecode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, papecode:
...
Jeśli serwer wymaga uwierzytelnienia, papecode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `papecode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji papecode bezpieczne przechowa tokeny w `~/.local/share/papecode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w papecode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie papecode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć papecode z kontem Sentry.

...
PapeCode Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
PapeCode Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w PapeCode. Subskrybujesz PapeCode Go i
...
PapeCode Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

PapeCode Go działa jak każdy inny dostawca w PapeCode.

1. Logujesz się do **<a href={console}>PapeCode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `PapeCode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować PapeCode Go.
:::
...
PapeCode Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji PapeCode
używa formatu `papecode-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`papecode-go/kimi-k2.6` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z PapeCode.

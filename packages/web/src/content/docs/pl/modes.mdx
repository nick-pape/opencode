...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji papecode.
...
te lub skonfiguruj własne za pomocą konfiguracji papecode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.papecode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `papecode.json`:
...
- Globalnie: `~/.config/papecode/modes/`
...
Jeśli nie ma wpływu na temperaturę, papecode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna papecode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.papecode/modes/` dla trybów zapisanych dla projektu lub `~/.config/papecode/modes/` dla trybów globalnych:

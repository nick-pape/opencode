---
title: Serwer
description: Interakcja z serwerem papecode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `papecode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient papecode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `papecode.local` |
...
Ustaw `PAPECODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `papecode` lub ustaw `PAPECODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `papecode serve`, jak i `papecode web`.
...
Kiedy uruchomisz `papecode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera papecode do programowej interakcji z papecode.
:::

Ta architektura umożliwia obsługę wielu klientów przez papecode i programową interakcję z kodem otwartym.

Możesz uruchomić `papecode serve`, aby uruchomić samodzielny serwer. Jeśli masz
papecode TUI działa, `papecode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki papecode [IDE](/docs/ide).
...
Serwer papecode udostępnia następujące interfejsy API.

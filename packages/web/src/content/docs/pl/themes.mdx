...
Dzięki papecode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie papecode używa naszego własnego motywu `papecode`.
...
papecode ma kilka wbudowanych motywów.
...
- Chcą, aby PapeCode pasował do wyglądu ich terminala
...
papecode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/papecode/themes/*.json` lub `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.papecode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.papecode/themes/*.json`

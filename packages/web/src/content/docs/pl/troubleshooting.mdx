...
Aby debugować problemy z papecode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\papecode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `papecode --log-level DEBUG`.

---

## Przechowywanie danych

papecode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/papecode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\papecode`
...
papecode Desktop uruchamia w tle lokalny serwer papecode (sidecar `papecode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `PapeCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/papecode/papecode.jsonc` (lub `~/.config/papecode/papecode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/papecode/papecode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
papecode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/papecode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\papecode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby papecode mógł ją odbudować.

1. Całkowicie zamknij papecode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/papecode`
- **Linux**: usuń `~/.cache/papecode` (lub uruchom `rm -rf ~/.cache/papecode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\papecode`

3. Uruchom ponownie papecode Desktop.

---

### Napraw problemy z połączeniem z serwerem

papecode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `papecode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `PAPECODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `PAPECODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows papecode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami papecode.

---

### Brak powiadomień

papecode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla papecode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie papecode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji papecode Desktop):

- `papecode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `papecode.global.dat` i `papecode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z papecode:
...
3. Upewnij się, że masz najnowszą wersję z `papecode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `papecode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\papecode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. papecode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\papecode`

2. Uruchom ponownie kod papecode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to papecode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
papecode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.

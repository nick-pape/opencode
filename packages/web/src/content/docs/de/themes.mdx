...
Mit PapeCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet PapeCode das `papecode`-Theme.
...
PapeCode liefert mehrere eingebaute Themes mit.
...
- PapeCode optisch ans Terminal anpassen wollen
...
PapeCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/papecode/themes/*.json` oder `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.papecode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.papecode/themes/*.json`

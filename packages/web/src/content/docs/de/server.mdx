---
title: Server
description: Nutze den papecode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `papecode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein papecode-Client nutzen kann.
...
Setze `PAPECODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `papecode`, kann aber mit `PAPECODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `papecode serve` und `papecode web`.
...
Wenn du `papecode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den papecode-Server, um papecode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `papecode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `papecode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die PapeCode-[IDE](/docs/ide)-Plugins.
...
Der papecode-Server stellt folgende APIs bereit.

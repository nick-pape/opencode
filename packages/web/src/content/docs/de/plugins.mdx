---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um PapeCode zu erweitern.
---

Mit Plugins können Sie PapeCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von PapeCode zu ändern.
...
- `.papecode/plugins/` – Plugins auf Projektebene
- `~/.config/papecode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/papecode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/papecode/papecode.json`)
2. Projektkonfiguration (`papecode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/papecode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.papecode/plugins/`)
...
PapeCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein PapeCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie PapeCode erweitern können.
...
:::note
Wenn Sie die PapeCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass PapeCode `.env`-Dateien liest:
...
Plugins können PapeCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das PapeCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für PapeCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://papecode.ai/docs/sdk).

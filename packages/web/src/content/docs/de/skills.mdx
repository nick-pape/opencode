...
Agent Skills erlauben PapeCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
PapeCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft PapeCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.papecode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.papecode/skills/git-release/SKILL.md` zum Beispiel so:
...
PapeCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `papecode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `papecode.json`):

...
description: Erstellen Sie Tools, die der LLM in PapeCode aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von PapeCode wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.papecode/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/papecode/tools/` platzieren.

...
Wenn PapeCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\papecode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `papecode --log-level DEBUG`.

---

## Speicher

papecode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/papecode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\papecode`
...
PapeCode Desktop startet im Hintergrund einen lokalen PapeCode-Server (`papecode-cli`-Sidecar).
...
- Nur macOS: `PapeCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/papecode/papecode.jsonc` (oder `~/.config/papecode/papecode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/papecode/papecode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
PapeCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/papecode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\papecode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit PapeCode ihn neu aufbauen kann.

1. Beende PapeCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/papecode`
- **Linux**: loesche `~/.cache/papecode` (oder fuehre aus: `rm -rf ~/.cache/papecode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\papecode`

3. Starte PapeCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

PapeCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `papecode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `PAPECODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `PAPECODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt PapeCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit PapeCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

PapeCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer PapeCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende PapeCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von PapeCode Desktop):

- `papecode.settings.dat` (desktop default server URL)
- `papecode.global.dat` und `papecode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit PapeCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `papecode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\papecode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. papecode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\papecode`

2. Starte papecode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt papecode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
papecode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).

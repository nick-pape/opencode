---
title: Regeln
description: Lege eigene Anweisungen fuer papecode fest.
---

Du kannst papecode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in papecode den Befehl `/init` aus.
...
So kann papecode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht papecode sie zu erweitern.
...
papecode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/papecode/AGENTS.md` speichern.
Diese gelten in allen papecode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt PapeCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/papecode/AGENTS.md` existiert)
...
Beim Start sucht papecode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/papecode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `papecode.json` oder in `~/.config/papecode/papecode.json` angeben.
...
papecode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit papecode.json

Empfohlen ist das `instructions`-Feld in der `papecode.json`:
...
Du kannst papecode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `papecode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

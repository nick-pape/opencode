---
title: LSP Server
description: PapeCode lässt sich in Ihren LSP-Server integrieren.
---

PapeCode lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

PapeCode verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `PAPECODE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn PapeCode eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer PapeCode-Konfiguration anpassen.

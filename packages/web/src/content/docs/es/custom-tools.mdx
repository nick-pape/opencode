...
description: Cree herramientas que LLM pueda llamar en papecode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de papecode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.papecode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/papecode/tools/`.
...
```ts title=".papecode/tools/database.ts" {1}
...
```ts title=".papecode/tools/math.ts"
import { tool } from "@papecode-ai/plugin"
...
```ts title=".papecode/tools/bash.ts"
import { tool } from "@papecode-ai/plugin"
...
```ts title=".papecode/tools/project.ts" {8}
import { tool } from "@papecode-ai/plugin"
...
```python title=".papecode/tools/add.py"
import sys
...
```ts title=".papecode/tools/python-add.ts" {10}
import { tool } from "@papecode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".papecode/tools/add.py")

...
Para depurar problemas con PapeCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/papecode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\papecode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `papecode --log-level DEBUG`.

---

## Almacenamiento

papecode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/papecode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\papecode`
...
PapeCode Desktop ejecuta un servidor PapeCode local (el sidecar `papecode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `PapeCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/papecode/papecode.jsonc` (o `~/.config/papecode/papecode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/papecode/papecode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
PapeCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/papecode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\papecode\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que PapeCode pueda reconstruirlo.

1. Salga de PapeCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/papecode`
- **Linux**: eliminar `~/.cache/papecode` (o ejecutar `rm -rf ~/.cache/papecode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\papecode`

3. Reinicie el escritorio PapeCode.

---

### Solucionar problemas de conexión del servidor

PapeCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `papecode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `PAPECODE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `PAPECODE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio PapeCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de PapeCode.

---

### Notificaciones que no se muestran

PapeCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para PapeCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio PapeCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio PapeCode):

- `papecode.settings.dat` (URL del servidor predeterminado de escritorio)
- `papecode.global.dat` y `papecode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con PapeCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/papecode/issues**](https://github.com/anomalyco/papecode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**papecode.ai/discord**](https://papecode.ai/discord)
...
3. Asegúrese de tener la última versión con `papecode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `papecode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/papecode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\papecode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. papecode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\papecode`

2. Reinicie papecode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a papecode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
papecode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.

...
Con PapeCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, PapeCode usa nuestro propio tema `papecode`.
...
PapeCode viene con varios temas integrados.
...
- Quiere que PapeCode coincida con la apariencia de su terminal
...
PapeCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/papecode/themes/*.json` o `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.papecode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.papecode/themes/*.json`

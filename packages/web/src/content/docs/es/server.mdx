---
title: Servidor
description: Interactuar con el servidor papecode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `papecode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente papecode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `papecode.local` |
...
Configure `PAPECODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `papecode`, o configure `PAPECODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `papecode serve` como a `papecode web`.
...
Cuando ejecuta `papecode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor papecode para interactuar con papecode mediante programación.
:::

Esta arquitectura permite que papecode admita múltiples clientes y le permite interactuar con papecode mediante programación.

Puede ejecutar `papecode serve` para iniciar un servidor independiente. Si tienes el
papecode TUI ejecutándose, `papecode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos PapeCode [IDE](/docs/ide).
...
El servidor papecode expone las siguientes API.

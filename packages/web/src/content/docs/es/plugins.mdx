---
title: Complementos
description: Escriba sus propios complementos para extender PapeCode.
---

Los complementos le permiten extender PapeCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de PapeCode.
...
- `.papecode/plugins/` - Complementos a nivel de proyecto
- `~/.config/papecode/plugins/` - Complementos globales
...
```json title="papecode.json"
{
  ...
  "plugin": ["papecode-helicone-session", "papecode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/papecode/node_modules/`.
...
1. Configuración global (`~/.config/papecode/papecode.json`)
2. Configuración del proyecto (`papecode.json`)
3. Directorio global de complementos (`~/.config/papecode/plugins/`)
4. Directorio de complementos del proyecto (`.papecode/plugins/`)
...
```json title=".papecode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
PapeCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".papecode/plugins/my-plugin.ts"
...
```js title=".papecode/plugins/example.js"
...
- `client`: Un cliente SDK papecode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar papecode.
...
```js title=".papecode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "papecode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio PapeCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que papecode lea archivos `.env`:

```javascript title=".papecode/plugins/env-protection.js"
...
```javascript title=".papecode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a papecode:

```ts title=".papecode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@papecode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que papecode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para papecode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://papecode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".papecode/plugins/compaction.ts"
import type { Plugin } from "@papecode-ai/plugin"
...
```ts title=".papecode/plugins/custom-compaction.ts"
import type { Plugin } from "@papecode-ai/plugin"

...
Las habilidades del agente permiten a PapeCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
PapeCode busca estas ubicaciones:

- Configuración del proyecto: `.papecode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/papecode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, PapeCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.papecode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.papecode/skills/git-release/SKILL.md` así:
...
PapeCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `papecode.json`:
...
**Para agentes integrados** (en `papecode.json`):

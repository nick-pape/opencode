...
Las herramientas permiten que LLM realice acciones en su código base. PapeCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en PapeCode.
...
:::note
Esta herramienta solo está disponible cuando `PAPECODE_EXPERIMENTAL_LSP_TOOL=true` (o `PAPECODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor PapeCode o cuando la variable de entorno `PAPECODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar PapeCode:

```bash
PAPECODE_ENABLE_EXA=1 papecode
```

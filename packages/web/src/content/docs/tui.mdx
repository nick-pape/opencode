---
title: TUI
description: Using the PapeCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PapeCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running PapeCode starts the TUI for the current directory.
...
When using the PapeCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to PapeCode. Allows you to select from available providers and add their API keys.
...
Exit PapeCode. _Aliases_: `/quit`, `/q`
...
This is separate from `papecode.json`, which configures server/runtime behavior.
...
Use `PAPECODE_TUI_CONFIG` to load a custom TUI config path.

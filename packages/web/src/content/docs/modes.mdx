...
:::caution
Modes are now configured through the `agent` option in the papecode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in papecode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the papecode config.
...
papecode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.papecode/plans/*.md` to detail the plan itself
...
Configure modes in your `papecode.json` config file:
...
both the global papecode config and the project specific config.
...
Create mode files in `.papecode/modes/` for project-specific modes or `~/.config/papecode/modes/` for global modes:

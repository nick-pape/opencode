...
Create `.papecode/commands/test.md`:
...
You can add custom commands through the PapeCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your PapeCode [config](/docs/config):
...
papecode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).

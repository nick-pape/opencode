...
For at fejlfinde problemer med PapeCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\papecode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `papecode --log-level DEBUG`.

---

## Lagring

papecode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/papecode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\papecode`
...
PapeCode Desktop kører en lokal PapeCode-server (`papecode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `PapeCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/papecode/papecode.jsonc` (eller `~/.config/papecode/papecode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/papecode/papecode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
PapeCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/papecode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\papecode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så PapeCode kan genopbygge den.

1. Afslut PapeCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/papecode`
- **Linux**: slet `~/.cache/papecode` (eller kør `rm -rf ~/.cache/papecode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\papecode`

3. Start PapeCode Desktop på ny.

---

### Løs problemer med serverforbindelse

PapeCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `papecode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `PAPECODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `PAPECODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver PapeCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i PapeCode.

---

### Meddelelser vises ikke

PapeCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for PapeCode i dine OS-indstillinger, og
...
1. Afslut PapeCode Desktop.
2. Find og slet disse filer (de findes i PapeCode Desktop-appens datamappe):

- `papecode.settings.dat` (skrivebordsstandardserver URL)
- `papecode.global.dat` og `papecode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med PapeCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `papecode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\papecode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. papecode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\papecode`

2. Start papecode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge papecode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
papecode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.

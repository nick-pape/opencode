---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i PapeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PapeCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i PapeCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/papecode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i PapeCode-konfigurationen.
...
PapeCode Zen er en liste over modeller leveret af PapeCode-teamet, som er blevet testet og verificeret til at fungere godt med PapeCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med PapeCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg papecode og gå til [papecode.ai/auth](https://papecode.ai/auth).
...
Det fungerer som alle andre udbydere i PapeCode og er helt valgfrit at bruge.

---

## PapeCode Go

PapeCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af PapeCode-teamet, som er testet og verificeret til at fungere godt med PapeCode.

1. Kør kommandoen `/connect` i TUI, vælg `PapeCode Go`, og gå til [papecode.ai/auth](https://papecode.ai/zen).
...
Sådan bruger du Amazon Bedrock med PapeCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører papecode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `papecode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i PapeCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at papecode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem papecode-konfigurationen.
...
gemme tokenet i papecode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
PapeCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse PapeCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`papecode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `papecode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med PapeCode:
...
Sådan bruger du Google Vertex AI med PapeCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører papecode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem papecode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) plugin for automatisk at logge hver PapeCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver PapeCode-samtale opført som en separat session.
...
Du kan konfigurere papecode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere papecode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere papecode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for PapeCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/papecode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med PapeCode:
...
3. Kopier API-nøglen til brug i PapeCode.
...
6. **Vigtigt**: Før du bruger skymodeller i PapeCode, skal du hente modelinformationen lokalt:
...
PapeCode Zen er en liste over testede og verificerede modeller leveret af PapeCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>PapeCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **PapeCode Zen**.

   ```txt
   /connect
   ```

3. Indtast PapeCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem papecode-konfigurationen.
...
5. Du kan også tilpasse dem gennem papecode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med PapeCode:
...
Du kan også tilpasse modeller gennem papecode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med papecode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `papecode.json`-filen i projektkataloget:
...
`limit`-felterne lader PapeCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `papecode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek papecode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i papecode-konfigurationen.

---
title: Server
description: Interager med papecode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `papecode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en papecode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `papecode.local` |
...
Indstil `PAPECODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `papecode`, eller indstil `PAPECODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `papecode serve` og `papecode web`.
...
Når du kører `papecode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug papecode-serveren til at interagere med papecode programmatisk.
:::

Denne arkitektur lader papecode understøtte flere klienter og lader dig interagere med papecode programmatisk.

Du kan køre `papecode serve` for at starte en selvstændig server. Hvis du har
papecode TUI kørende, vil `papecode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af PapeCode [IDE](/docs/ide) plugins.
...
PapeCode-serveren viser følgende API'er.

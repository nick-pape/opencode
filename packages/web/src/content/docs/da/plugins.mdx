---
title: Plugins
description: Skriv dine egne plugins for at udvide PapeCode.
---

Plugins giver dig mulighed for at udvide PapeCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre PapeCodes standardadfærd.
...
- `.papecode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/papecode/node_modules/`.
...
1. Global konfiguration (`~/.config/papecode/papecode.json`)
2. Projektkonfiguration (`papecode.json`)
3. Global plugin-mappe (`~/.config/papecode/plugins/`)
4. Projekt plugin bibliotek (`.papecode/plugins/`)
...
PapeCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En papecode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide papecode.
...
:::note
Hvis du bruger PapeCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå papecode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til papecode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som papecode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for papecode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://papecode.ai/docs/sdk) for detaljer.

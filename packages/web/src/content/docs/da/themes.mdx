...
Med PapeCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger PapeCode vårt eget `papecode`-tema.
...
PapeCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/papecode/themes/*.json` eller `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.papecode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.papecode/themes/*.json`

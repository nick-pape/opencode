...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i papecode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i papecode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem papecode-konfigurationen.
...
papecode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.papecode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `papecode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger papecode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale papecode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.papecode/modes/` for projektspecifikke tilstande eller `~/.config/papecode/modes/` for globale tilstande:

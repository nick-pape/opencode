...
Du kan tilføje eksterne værktøjer til PapeCode ved hjælp af _Model Context Protocol_ eller MCP. PapeCode understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [PapeCode Config](https://papecode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/papecode`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
PapeCode håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil PapeCode:
...
Hvis serveren kræver godkendelse, vil PapeCode bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `papecode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil PapeCode opbevare tokens sikkert i `~/.local/share/papecode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem PapeCode-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde PapeCode til din Sentry-konto.

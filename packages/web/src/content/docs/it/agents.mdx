...
In PapeCode esistono due tipi di agenti: agenti primari e subagenti.
...
PapeCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
PapeCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

PapeCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `papecode.json`:
...
- Globale: `~/.config/papecode/agents/`
- Per progetto: `.papecode/agents/`
...
Se non specifichi una temperature, PapeCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di PapeCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione PapeCode usa il formato `provider/model-id`. Per esempio, se stai usando [PapeCode Zen](/docs/zen), useresti `papecode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `papecode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/papecode).
:::

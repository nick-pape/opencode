...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. PapeCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in PapeCode.
...
:::note
Questo strumento e' disponibile solo quando `PAPECODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `PAPECODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider PapeCode o quando la variabile d'ambiente `PAPECODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di PapeCode:

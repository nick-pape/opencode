---
title: Plugin
description: Scrivi plugin per estendere PapeCode.
---

I plugin ti permettono di estendere PapeCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di PapeCode.
...
- `.papecode/plugins/` - plugin a livello progetto
- `~/.config/papecode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/papecode/node_modules/`.
...
1. Config globale (`~/.config/papecode/papecode.json`)
2. Config di progetto (`papecode.json`)
3. Directory plugin globale (`~/.config/papecode/plugins/`)
4. Directory plugin di progetto (`.papecode/plugins/`)
...
PapeCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di papecode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere papecode.
...
:::note
Se usi l'app desktop di PapeCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a papecode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a papecode:
...
L'helper `tool` crea uno strumento personalizzato che papecode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in papecode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://papecode.ai/docs/sdk) per i dettagli.

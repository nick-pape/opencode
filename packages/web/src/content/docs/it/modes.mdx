...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di papecode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in papecode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di papecode.
...
papecode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.papecode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `papecode.json`:
...
- Globale: `~/.config/papecode/modes/`
- Progetto: `.papecode/modes/`
...
Se non viene specificata alcuna temperatura, papecode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di papecode sia per quella specifica del progetto.
...
Crea file di modalita in `.papecode/modes/` per modalita specifiche del progetto oppure in `~/.config/papecode/modes/` per modalita globali:

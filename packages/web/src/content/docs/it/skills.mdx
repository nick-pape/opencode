...
Le skill degli agenti permettono a PapeCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
PapeCode cerca in queste posizioni:

- Config di progetto: `.papecode/skills/<name>/SKILL.md`
- Config globale: `~/.config/papecode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, PapeCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.papecode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.papecode/skills/git-release/SKILL.md` cosi':
...
PapeCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `papecode.json`:
...
**Per agenti integrati** (in `papecode.json`):

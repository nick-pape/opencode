---
title: Server
description: Interagisci con il server papecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `papecode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client papecode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `papecode.local` |
...
Imposta `PAPECODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `papecode`, oppure imposta `PAPECODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `papecode serve` sia per `papecode web`.
...
Quando esegui `papecode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server papecode per interagire con papecode in modo programmatico.
:::

Questa architettura permette a papecode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `papecode serve` per avviare un server standalone. Se la TUI di papecode e gia in esecuzione, `papecode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di PapeCode.
...
Il server papecode espone le seguenti API.

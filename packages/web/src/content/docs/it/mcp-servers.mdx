...
Puoi aggiungere strumenti esterni a PapeCode usando il _Model Context Protocol_, o MCP. PapeCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [PapeCode Config](https://papecode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/papecode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
PapeCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, PapeCode:
...
Se il server richiede autenticazione, PapeCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `papecode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, PapeCode memorizzera' i token in modo sicuro in `~/.local/share/papecode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in PapeCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione PapeCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare PapeCode al tuo account Sentry.

...
Con PapeCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, PapeCode usa il tema `papecode`.
...
PapeCode include diversi temi integrati.
...
- Vuole che PapeCode corrisponda all'aspetto del terminale
...
PapeCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/papecode/themes/*.json` o `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.papecode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.papecode/themes/*.json`

---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di PapeCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PapeCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire PapeCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di PapeCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a PapeCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da PapeCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `papecode.json`, che configura il comportamento di server/runtime.
...
Usa `PAPECODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.

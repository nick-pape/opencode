...
PapeCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
PapeCode Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in PapeCode. Ti abboni a PapeCode Go e
...
PapeCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

PapeCode Go funziona come qualsiasi altro provider in PapeCode.

1. Accedi a **<a href={console}>PapeCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `PapeCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a PapeCode Go.
:::
...
PapeCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua PapeCode config
utilizza il formato `papecode-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`papecode-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato PapeCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con PapeCode.

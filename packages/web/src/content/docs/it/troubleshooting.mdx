...
Per diagnosticare problemi con PapeCode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\papecode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `papecode --log-level DEBUG`.

---

## Archiviazione

papecode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/papecode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\papecode`
...
PapeCode Desktop esegue in background un server locale di PapeCode (il sidecar `papecode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `PapeCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/papecode/papecode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
PapeCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/papecode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\papecode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che PapeCode possa ricostruirla.

1. Chiudi completamente PapeCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/papecode`
- **Linux**: elimina `~/.cache/papecode` (oppure esegui `rm -rf ~/.cache/papecode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\papecode`

3. Riavvia PapeCode Desktop.

---

### Risolvi problemi di connessione al server

PapeCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `papecode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `PAPECODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `PAPECODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, PapeCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di PapeCode.

---

### Notifiche non visualizzate

PapeCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per PapeCode nelle impostazioni del sistema operativo, e
...
1. Chiudi PapeCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app PapeCode Desktop):

- `papecode.settings.dat` (desktop default server URL)
- `papecode.global.dat` e `papecode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con PapeCode:
...
Per capire a quali modelli hai accesso, esegui `papecode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\papecode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. papecode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\papecode`

2. Riavvia papecode per reinstallare i pacchetti provider piu recenti

Questo forzera papecode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
papecode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.

...
:::caution
モードは、PapeCode 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、PapeCode 設定を通じて独自の設定を行います。
...
PapeCode には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.papecode/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`papecode.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/papecode/modes/`
- プロジェクト: `.papecode/modes/`
...
温度が指定されていない場合、PapeCode はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.papecode/modes/` に、グローバルモードの場合は `~/.config/papecode/modes/` に作成します。

...
エージェントスキルにより、PapeCode はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
PapeCode は次の場所を検索します。

- プロジェクト設定: `.papecode/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/papecode/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、PapeCode は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.papecode/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/papecode/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.papecode/skills/git-release/SKILL.md` を作成します。
...
PapeCode では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`papecode.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。

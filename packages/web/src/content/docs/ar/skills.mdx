...
تتيح مهارات الوكيل لـ PapeCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث PapeCode في هذه المواقع:

- إعدادات المشروع: `.papecode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/papecode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج PapeCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.papecode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/papecode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.papecode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض PapeCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `papecode.json`:
...
**للوكلاء المدمجين** (في `papecode.json`):

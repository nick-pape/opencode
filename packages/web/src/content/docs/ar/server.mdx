---
title: الخادم
description: تفاعل مع خادم papecode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `papecode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل papecode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `papecode.local` |
...
عيّن `PAPECODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `papecode`، أو عيّن `PAPECODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `papecode serve` و `papecode web`.
...
عند تشغيل `papecode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم papecode للتفاعل مع papecode برمجيا.
:::

تتيح هذه البنية لـ papecode دعم عدة عملاء وتمكّنك من التفاعل مع papecode برمجيا.

يمكنك تشغيل `papecode serve` لبدء خادم مستقل. إذا كانت واجهة papecode في terminal (TUI)
قيد التشغيل، فسيبدأ `papecode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات PapeCode لـ [IDE](/docs/ide).
...
يعرض خادم papecode واجهات API التالية.

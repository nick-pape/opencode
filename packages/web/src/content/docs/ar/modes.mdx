...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات papecode. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في papecode تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات papecode.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.papecode/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `papecode.json` لديك:
...
- عالمي: `~/.config/papecode/modes/`
- للمشروع: `.papecode/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم papecode القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات papecode العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.papecode/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/papecode/modes/` لأوضاع عالمية:

...
مع PapeCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم PapeCode سمتنا `papecode`.
...
يأتي PapeCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق PapeCode مظهر terminal لديهم
...
يدعم PapeCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/papecode/themes/*.json` أو `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.papecode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.papecode/themes/*.json`

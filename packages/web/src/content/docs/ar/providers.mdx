---
title: المزوّدون
description: استخدام أي موفّر LLM في PapeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم PapeCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات PapeCode.
...
في `~/.local/share/papecode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات PapeCode.
...
PapeCode Zen هي قائمة نماذج يوفّرها فريق PapeCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع PapeCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع PapeCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر papecode، ثم انتقل إلى [papecode.ai/auth](https://papecode.ai/auth).
...
يعمل مثل أي مزوّد آخر في PapeCode واستخدامه اختياري بالكامل.

---

## PapeCode Go

PapeCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق PapeCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع PapeCode.

1. شغّل الأمر `/connect` في TUI، واختر `PapeCode Go`، ثم انتقل إلى [papecode.ai/auth](https://papecode.ai/zen).
...
لاستخدام Amazon Bedrock مع PapeCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل papecode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `papecode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل PapeCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل papecode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات papecode.
...
تخزين الرمز في مساحة تخزين مصادقة papecode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم PapeCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر PapeCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`papecode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth).

##### التهيئة

خصّص عبر `papecode.json`:
...
لاستخدام اشتراك GitHub Copilot مع papecode:
...
لاستخدام Google Vertex AI مع PapeCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر papecode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) لتسجيل كل محادثة PapeCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة PapeCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط papecode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط papecode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط papecode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع PapeCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/papecode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع PapeCode:
...
3. انسخ مفتاح API لاستخدامه في PapeCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في PapeCode، يجب سحب معلومات النموذج محليا:
...
PapeCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق PapeCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>PapeCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **PapeCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ PapeCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات papecode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات papecode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Papecode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات papecode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع papecode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `papecode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ PapeCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `papecode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات papecode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات papecode.

...
أنشئ الملف `.papecode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات PapeCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ PapeCode:
...
- على مستوى النظام: `~/.config/papecode/commands/`
- لكل مشروع: `.papecode/commands/`
...
يتضمن papecode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).

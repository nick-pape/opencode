...
With PapeCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, PapeCode uses our own `papecode` theme.
...
PapeCode comes with several built-in themes.
...
- Want PapeCode to match their terminal's appearance
...
PapeCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/papecode/themes/*.json` or `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.papecode/themes/*.json`
4. **Current working directory** - Defined in `./.papecode/themes/*.json`

---
title: TUI
description: PapeCode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

PapeCode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

PapeCode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
PapeCode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
PapeCode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
PapeCode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `papecode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `PAPECODE_TUI_CONFIG` kullanın.

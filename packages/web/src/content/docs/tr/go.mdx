...
PapeCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
PapeCode Go şu anda beta olarak mevcut.
:::

Go, PapeCode'daki diğer sağlayıcılar gibi çalışır. PapeCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve PapeCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
PapeCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

PapeCode Go, PapeCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>PapeCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `PapeCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye PapeCode Go'ya abone olabilir.
:::
...
PapeCode Go aşağıdaki limitleri içerir:
...
PapeCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `papecode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `papecode-go/kimi-k2.6` kullanmalısınız.
...
PapeCode Go'yu şu amaçlarla oluşturduk:
...
4. PapeCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.

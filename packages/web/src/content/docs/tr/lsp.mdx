...
description: PapeCode, LSP sunucularınızla bütünleşir.
---

PapeCode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

PapeCode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`PAPECODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
papecode bir dosyayı açtığında:
...
LSP sunucularını, papecode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.

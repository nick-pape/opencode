...
papecode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
papecode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
papecode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

papecode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `papecode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse papecode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global papecode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
papecode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [PapeCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `papecode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `papecode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/papecode).
:::

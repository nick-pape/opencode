...
:::caution
Modlar artık papecode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

papecode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya papecode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
papecode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.papecode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`papecode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, papecode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global papecode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.papecode/modes/` veya genel modlar için `~/.config/papecode/modes/`'de mod dosyaları oluşturun:

---
title: Sunucu
description: papecode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`papecode serve` komutu, papecode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `papecode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `PAPECODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `papecode` değeridir; değiştirmek isterseniz `PAPECODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `papecode serve` hem de `papecode web` için geçerlidir.
...
`papecode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
papecode ile programatik etkileşim için papecode sunucusunu kullanın.
:::

Bu mimari, papecode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`papecode serve` ile bağımsız bir sunucu başlatabilirsiniz. papecode TUI açıksa, `papecode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum papecode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
papecode sunucusu aşağıdaki API'leri sunar.

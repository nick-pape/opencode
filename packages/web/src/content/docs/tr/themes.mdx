...
papecode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak papecode kendi `papecode` temasını kullanır.
...
papecode birden fazla yerleşik temayla gelir.
...
- papecode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
papecode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/papecode/themes/*.json` veya `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.papecode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.papecode/themes/*.json`

...
_Model Bağlam Protokolü_ veya MCP kullanarak papecode'a harici araçlar ekleyebilirsiniz. papecode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [papecode Config](https://papecode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/papecode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
papecode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde papecode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa papecode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `papecode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından papecode, belirteçleri `~/.local/share/papecode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra papecode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi papecode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve papecode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.

---
title: Eklentiler
description: PapeCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek PapeCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya PapeCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.papecode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/papecode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/papecode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/papecode/papecode.json`)
2. Project config (`papecode.json`)
3. Global eklenti dizini (`~/.config/papecode/plugins/`)
4. Proje eklenti dizini (`.papecode/plugins/`)
...
PapeCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir PapeCode SDK istemcisi.
...
PapeCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
PapeCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
PapeCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca PapeCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, PapeCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra PapeCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://papecode.ai/docs/sdk) bakın.

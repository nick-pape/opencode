...
Ajan becerileri, papecode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
papecode su konumlari tarar:

- Proje konfigurasyonu: `.papecode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/papecode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda papecode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.papecode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.papecode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
papecode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`papecode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`papecode.json` icinde):

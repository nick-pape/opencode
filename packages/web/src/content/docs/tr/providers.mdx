---
title: Sağlayıcılar
description: papecode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

papecode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı papecode yapılandırmanızda yapılandırın.
...
`~/.local/share/papecode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları papecode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
PapeCode Zen, papecode ekibi tarafından sağlanan modellerin bir listesidir.
papecode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz PapeCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, papecode'u seçin ve [papecode.ai/auth](https://papecode.ai/auth)'ye gidin.
...
papecode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## PapeCode Go

PapeCode Go, papecode ile iyi çalıştığı test edilmiş ve doğrulanmış, papecode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `PapeCode Go`'yu seçin ve [papecode.ai/auth](https://papecode.ai/zen) adresine gidin.
...
Amazon Bedrock'u papecode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   papecode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `papecode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi papecode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   papecode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Papecode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in papecode auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
papecode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. papecode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`papecode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`papecode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi papecode'la kullanmak için:
...
Google Vertex AI'yi papecode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları papecode çalıştırılırken ayarlayın.
...
Helicone'un papecode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her papecode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her papecode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
papecode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla papecode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla papecode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini papecode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/papecode)'a bakın.
:::
...
Ollama Cloud'u papecode ile kullanmak için:
...
3. papecode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: papecode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
PapeCode Zen, papecode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>PapeCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **PapeCode Zen**'i arayın.

   ```txt
   /connect
   ```

3. papecode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   papecode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca papecode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Papecode ile kullanmak için:
...
Modelleri papecode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı papecode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `papecode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları papecode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `papecode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için papecode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, papecode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.

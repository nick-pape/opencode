...
Il existe deux types d'agents dans PapeCode ; agents primaires et sous-agents.
...
PapeCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
PapeCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

PapeCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `papecode.json` :
...
- Global : `~/.config/papecode/agents/`
- Par projet : `.papecode/agents/`

```markdown title="~/.config/papecode/agents/review.md"
---
...
Si aucune température n'est spécifiée, PapeCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale PapeCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration PapeCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [PapeCode Zen](/docs/zen), vous utiliserez `papecode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="papecode.json" {3-6,9-12}
{
  "$schema": "https://papecode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="papecode.json" {3-5,8-10}
{
  "$schema": "https://papecode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `papecode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/papecode).
:::
...
```markdown title="~/.config/papecode/agents/docs-writer.md"
---
...
```markdown title="~/.config/papecode/agents/security-auditor.md"
---

---
title: Plugins
description: Écrivez vos propres plugins pour étendre PapeCode.
---

Les plugins vous permettent d'étendre PapeCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de PapeCode.
...
- `.papecode/plugins/` - Plugins au niveau du projet
- `~/.config/papecode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/papecode/node_modules/`.
...
1. Configuration globale (`~/.config/papecode/papecode.json`)
2. Configuration du projet (`papecode.json`)
3. Répertoire global des plugins (`~/.config/papecode/plugins/`)
4. Répertoire des plugins du projet (`.papecode/plugins/`)
...
PapeCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK papecode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre papecode.
...
:::note
Si vous utilisez l'application de bureau PapeCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez papecode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à papecode :
...
L'assistant `tool` crée un outil personnalisé que papecode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour papecode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://papecode.ai/docs/sdk) pour plus de détails.

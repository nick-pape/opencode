...
Pour déboguer les problèmes avec PapeCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\papecode\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `papecode --log-level DEBUG`.

---

## Stockage

papecode stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/papecode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\papecode`
...
PapeCode Desktop exécute un serveur PapeCode local (le side-car `papecode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `PapeCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/papecode/papecode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\papecode\papecode.jsonc`
...
PapeCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/papecode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\papecode\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que PapeCode puisse le reconstruire.

1. Quittez complètement PapeCode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/papecode`
- **Linux** : supprimez `~/.cache/papecode` (ou exécutez `rm -rf ~/.cache/papecode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\papecode`

3. Redémarrez le bureau PapeCode.

---

### Résoudre les problèmes de connexion au serveur

PapeCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `papecode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `PAPECODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `PAPECODE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, PapeCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de PapeCode.

---

### Les notifications ne s'affichent pas

PapeCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour PapeCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau PapeCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application PapeCode Desktop) :

- `papecode.settings.dat` (serveur de bureau par défaut URL)
- `papecode.global.dat` et `papecode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec PapeCode :
...
3. Assurez-vous d'avoir la dernière version avec `papecode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `papecode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\papecode`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. papecode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\papecode`

2. Redémarrez papecode pour réinstaller les derniers packages du fournisseur

Cela forcera papecode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
papecode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.

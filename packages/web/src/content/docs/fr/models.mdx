...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez PapeCode.
...
Voici plusieurs modèles qui fonctionnent bien avec PapeCode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration PapeCode.

```json title="papecode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [PapeCode Zen](/docs/zen), vous utiliserez `papecode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="papecode.jsonc" {7-12,19-24}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="papecode.jsonc" {6-21}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "papecode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. PapeCode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

PapeCode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="papecode.jsonc" {7-18}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque PapeCode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration PapeCode.

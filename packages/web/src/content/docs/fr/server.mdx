---
title: Serveur
description: Interagissez avec le serveur papecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `papecode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client papecode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `papecode.local` |
...
Définissez `PAPECODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `papecode`, ou définissez `PAPECODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `papecode serve` et à `papecode web`.
...
Lorsque vous exécutez `papecode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur papecode pour interagir avec papecode par programmation.
:::

Cette architecture permet à papecode de prendre en charge plusieurs clients et vous permet d'interagir avec papecode par programme.

Vous pouvez exécuter `papecode serve` pour démarrer un serveur autonome. Si vous avez le
papecode TUI en cours d'exécution, `papecode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins PapeCode [IDE](/docs/ide).
...
Le serveur papecode expose les API suivantes.

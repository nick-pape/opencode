---
title: SDK
description: Client JS de type sécurisé pour le serveur papecode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK papecode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler papecode par programme.
...
Créez une instance de papecode :

```javascript
import { createPapecode } from "@papecode-ai/sdk"

const { client } = await createPapecode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `papecode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${papecode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de papecode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@papecode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*papecode" },
})

...
Avec PapeCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, PapeCode utilise notre propre thème `papecode`.
...
PapeCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que PapeCode corresponde à l'apparence de leur terminal
...
PapeCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/papecode/themes/*.json` ou `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.papecode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.papecode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/papecode/themes
vim ~/.config/papecode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .papecode/themes
vim .papecode/themes/my-theme.json
```
...
{
  "$schema": "https://papecode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

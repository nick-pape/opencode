﻿---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en PapeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PapeCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration PapeCode.
...
en `~/.local/share/papecode/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre PapeCode
configuration.
...
PapeCode Zen est une liste de modèles fournis par l'équipe PapeCode qui ont été
testé et vérifié pour fonctionner correctement avec PapeCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par PapeCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez papecode et dirigez-vous vers [papecode.ai/auth](https://papecode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans PapeCode et son utilisation est totalement facultative.

---

## PapeCode Go

PapeCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe PapeCode qui ont été
testé et vérifié pour fonctionner correctement avec PapeCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `PapeCode Go` et rendez-vous sur [papecode.ai/auth](https://papecode.ai/zen).
...
Pour utiliser Amazon Bedrock avec PapeCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de papecode :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY papecode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile papecode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX papecode
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `papecode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans PapeCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que papecode fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX papecode
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX papecode
   ```
...
Vous pouvez également ajouter des modèles via votre configuration papecode.
...
pour stocker le jeton dans le stockage d'authentification papecode.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
PapeCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller PapeCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `papecode.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth).

##### Configuration

Personnalisez via `papecode.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec papecode :
...
Pour utiliser Google Vertex AI avec PapeCode :
...
Définissez-les lors de l'exécution de papecode.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id papecode
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via papecode, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/papecode/papecode.jsonc"
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) pour enregistrer automatiquement chaque conversation PapeCode en tant que session dans Helicone.

```bash
npm install -g papecode-helicone-session
```
...
```json title="papecode.json"
{
  "plugin": ["papecode-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation PapeCode répertoriée comme une session distincte.
...
Vous pouvez configurer papecode pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="papecode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer papecode pour utiliser des modèles locaux via LM Studio.

```json title="papecode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer papecode pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour PapeCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/papecode) pour plus de détails.
:::

```json title="papecode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec PapeCode :
...
3. Copiez la clé API à utiliser dans PapeCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans PapeCode, vous devez extraire les informations du modèle localement :
...
PapeCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe PapeCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>PapeCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **PapeCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé PapeCode API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration papecode.
...
5. Vous pouvez également les personnaliser via votre configuration papecode. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' papecode
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group papecode
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Papecode :
...
Vous pouvez également personnaliser les modèles via votre configuration papecode. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec papecode. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in papecode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `papecode.json` dans le répertoire de votre projet :

   ```json title="papecode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://papecode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="papecode.json" {9,11,17-20}
{
  "$schema": "https://papecode.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à PapeCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `papecode auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration papecode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration papecode.

...
Les compétences d'agent permettent à PapeCode de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
PapeCode recherche ces emplacements :

- Configuration du projet : `.papecode/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/papecode/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, PapeCode remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.papecode/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.papecode/skills/git-release/SKILL.md` comme ceci :
...
PapeCode répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `papecode.json` :
...
**Pour les agents intégrés** (dans `papecode.json`) :

...
There are two types of agents in PapeCode; primary agents and subagents.
...
PapeCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
PapeCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

PapeCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `papecode.json` config file:
...
If no temperature is specified, PapeCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global PapeCode config and the project specific config.
...
The model ID in your PapeCode config uses the format `provider/model-id`. For example, if you're using [PapeCode Zen](/docs/zen), you would use `papecode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `papecode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/papecode).
:::

---
title: Server
description: Interact with papecode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `papecode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an papecode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `papecode.local` |
...
Set `PAPECODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `papecode`, or set `PAPECODE_SERVER_USERNAME` to override it. This applies to both `papecode serve` and `papecode web`.
...
When you run `papecode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the papecode server to interact with papecode programmatically.
:::

This architecture lets papecode support multiple clients and allows you to interact with papecode programmatically.

You can run `papecode serve` to start a standalone server. If you have the
papecode TUI running, `papecode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the PapeCode [IDE](/docs/ide) plugins.
...
The papecode server exposes the following APIs.

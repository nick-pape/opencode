---
title: Providers
description: Using any LLM provider in PapeCode.
---

import config from "../../../config.mjs"
export const console = config.console

PapeCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your PapeCode config.
...
You can customize the providers through the `provider` section in your PapeCode
config.
...
PapeCode Zen is a list of models provided by the PapeCode team that have been
tested and verified to work well with PapeCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with PapeCode Zen.
:::

1. Run the `/connect` command in the TUI, select `PapeCode Zen`, and head to [papecode.ai/auth](https://papecode.ai/zen).
...
It works like any other provider in PapeCode and is completely optional to use.

---

## PapeCode Go

PapeCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the PapeCode team that have been
tested and verified to work well with PapeCode.

1. Run the `/connect` command in the TUI, select `PapeCode Go`, and head to [papecode.ai/auth](https://papecode.ai/zen).
...
To use Amazon Bedrock with PapeCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running papecode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `papecode.json`:
...
PapeCode. Anthropic explicitly prohibits this.

Previous versions of PapeCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in PapeCode with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for papecode to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your papecode config.
...
GitLab Duo support in PapeCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

PapeCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
PapeCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock PapeCode
to only use your own GitLab-hosted instance, add the following to your
`papecode.json` file. It is also recommended to disable session sharing.
...
More documentation on [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, PapeCode will:
...
4. Route tool execution requests through PapeCode's permission-gated tool system
...
To use your GitHub Copilot subscription with papecode:
...
To use Google Vertex AI with PapeCode:
...
2. Set the required environment variables:
   ...
   Set them while running papecode.
...
In the event you see a feature or model from Helicone that isn't configured automatically through papecode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) plugin to automatically log each PapeCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each PapeCode conversation listed as a separate session.
...
You can configure papecode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure papecode to use local models through LM Studio.
...
You can configure papecode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for PapeCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/papecode) for details.
:::
...
To use Ollama Cloud with PapeCode:
...
3. Copy the API key for use in PapeCode.
...
6. **Important**: Before using cloud models in PapeCode, you must pull the model information locally:
...
PapeCode Zen is a list of tested and verified models provided by the PapeCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>PapeCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **PapeCode Zen**.

   ```txt
   /connect
   ```

3. Enter your PapeCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your papecode config.
...
5. You can also customize them through your papecode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Papecode:
...
You can also customize models through your papecode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with papecode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `papecode.json` file in your project directory:
...
The `limit` fields allow PapeCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `papecode auth list` to see if the credentials
...
2. For custom providers, check the papecode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your papecode config.

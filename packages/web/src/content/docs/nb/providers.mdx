---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i PapeCode.
---

import config from "../../../../config.mjs"
export const console = config.console

PapeCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i PapeCode-konfigurasjonen.
...
i `~/.local/share/papecode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i PapeCode
konfig.
...
PapeCode Zen er en liste over modeller levert av PapeCode-teamet som har vært
testet og verifisert for å fungere godt med PapeCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med PapeCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `PapeCode Zen` og gå til [papecode.ai/auth](https://papecode.ai/zen).
...
Det fungerer som alle andre leverandører i PapeCode og er helt valgfritt å bruke.

---

## PapeCode Go

PapeCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av PapeCode-teamet som har vært
testet og verifisert for å fungere godt med PapeCode.

1. Kjør kommandoen `/connect` i TUI, velg `PapeCode Go`, og gå til [papecode.ai/auth](https://papecode.ai/zen).
...
Slik bruker du Amazon Bedrock med PapeCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører PapeCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `papecode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i papecode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at PapeCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom PapeCode-konfigurasjonen.
...
for å lagre token i PapeCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
PapeCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse PapeCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`papecode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `papecode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med PapeCode:
...
Slik bruker du Google Vertex AI med PapeCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører PapeCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom PapeCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) plugin for automatisk å logge hver PapeCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver PapeCode-samtale oppført som en separat økt.
...
Du kan konfigurere PapeCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere PapeCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere PapeCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for PapeCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/papecode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med PapeCode:
...
3. Kopier API-nøkkelen for bruk i PapeCode.
...
6. **Viktig**: Før du bruker skymodeller i PapeCode, må du hente modellinformasjonen lokalt:
...
PapeCode Zen er en liste over testede og verifiserte modeller levert av PapeCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>PapeCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **PapeCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn PapeCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom PapeCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom PapeCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med PapeCode:
...
Du kan også tilpasse modeller gjennom PapeCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med PapeCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `papecode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar PapeCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `papecode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk PapeCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i PapeCode-konfigurasjonen.

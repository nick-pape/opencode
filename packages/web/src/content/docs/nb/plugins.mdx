...
description: Skriv dine egne programtillegg for å utvide PapeCode.
---

Plugins lar deg utvide PapeCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til PapeCode.
...
- `.papecode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/papecode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/papecode/papecode.json`)
2. Prosjektkonfigurasjon (`papecode.json`)
3. Global plugin-katalog (`~/.config/papecode/plugins/`)
4. Prosjektpluginkatalog (`.papecode/plugins/`)
...
PapeCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En PapeCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide PapeCode.
...
:::note
Hvis du bruker PapeCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre PapeCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til PapeCode:
...
`tool`-hjelperen lager et tilpasset verktøy som PapeCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for PapeCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://papecode.ai/docs/sdk) for detaljer.

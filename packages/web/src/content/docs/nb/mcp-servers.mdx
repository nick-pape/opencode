...
Du kan legge til eksterne verktøy i PapeCode ved å bruke _Model Context Protocol_ eller MCP. PapeCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [PapeCode Config](https://papecode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="papecode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/papecode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
PapeCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil PapeCode:
...
Hvis serveren krever autentisering, vil PapeCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `papecode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil PapeCode lagre tokens sikkert i `~/.local/share/papecode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i PapeCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom PapeCode-konfigurasjonen som alle andre verktøy.
...
```json title="papecode.json" {11, 14-18}
{
  "$schema": "https://papecode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble PapeCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver PapeCode-samtale oppført som en separat økt.

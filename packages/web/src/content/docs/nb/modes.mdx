...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i PapeCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i PapeCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom PapeCode-konfigurasjonen.
...
PapeCode kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.papecode/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `papecode.json` konfigurasjonsfil:
...
- Globalt: `~/.config/papecode/modes/`
- Prosjekt: `.papecode/modes/`
...
Hvis ingen temperatur er spesifisert, bruker PapeCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale PapeCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.papecode/modes/` for prosjektspesifikke moduser eller `~/.config/papecode/modes/` for globale moduser:

---
title: Tjener
description: Samhandle med PapeCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `papecode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en PapeCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `papecode.local` |
...
Sett `PAPECODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `papecode` som standard, eller sett `PAPECODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `papecode serve` og `papecode web`.
...
Når du kjører `papecode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk PapeCode-serveren til å samhandle med PapeCode programmatisk.
:::

Denne arkitekturen lar PapeCode støtte flere klienter og lar deg samhandle med PapeCode programmatisk.

Du kan kjøre `papecode serve` for å starte en frittstående server. Hvis du har
PapeCode TUI kjører, vil `papecode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av PapeCode [IDE](/docs/ide) plugins.
...
papecode-serveren viser følgende APIer.

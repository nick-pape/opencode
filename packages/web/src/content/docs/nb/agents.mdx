...
Det er to typer agenter i PapeCode; primære agenter og underagenter.
...
PapeCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
PapeCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

PapeCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `papecode.json`:
...
- Globalt: `~/.config/papecode/agents/`
- Per prosjekt: `.papecode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker PapeCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale PapeCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i PapeCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [PapeCode Zen](/docs/zen), vil du bruke `papecode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `papecode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/papecode).
:::

...
You can add external tools to PapeCode using the _Model Context Protocol_, or MCP. PapeCode supports both local and remote servers.
...
You can define MCP servers in your [PapeCode Config](https://papecode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/papecode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
PapeCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, PapeCode will:
...
If the server requires authentication, PapeCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `papecode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, PapeCode will store the tokens securely in `~/.local/share/papecode/mcp-auth.json`.
...
Your MCPs are available as tools in PapeCode, alongside built-in tools. So you can manage them through the PapeCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect PapeCode to your Sentry account.

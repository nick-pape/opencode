---
title: Plugins
description: Escreva seus próprios plugins para estender o papecode.
---

Plugins permitem que você estenda o papecode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do papecode.
...
- `.papecode/plugins/` - Plugins em nível de projeto
- `~/.config/papecode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/papecode/node_modules/`.
...
1. Configuração global (`~/.config/papecode/papecode.json`)
2. Configuração do projeto (`papecode.json`)
3. Diretório de plugins global (`~/.config/papecode/plugins/`)
4. Diretório de plugins do projeto (`.papecode/plugins/`)
...
O papecode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do papecode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o papecode.
...
:::note
Se você estiver usando o aplicativo desktop papecode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o papecode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao papecode:
...
O helper `tool` cria uma ferramenta personalizada que o papecode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o papecode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://papecode.ai/docs/sdk) para detalhes.

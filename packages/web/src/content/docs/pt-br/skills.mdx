...
As habilidades do agente permitem que o papecode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O papecode pesquisa nesses locais:

- Configuração do projeto: `.papecode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/papecode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o papecode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.papecode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/papecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.papecode/skills/git-release/SKILL.md` assim:
...
O papecode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `papecode.json`:
...
**Para agentes embutidos** (em `papecode.json`):

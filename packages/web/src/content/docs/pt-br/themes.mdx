...
Com o papecode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o papecode usa nosso próprio tema `papecode`.
...
O papecode vem com vários temas embutidos.
...
- Querem que o papecode corresponda à aparência do seu terminal
...
O papecode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/papecode/themes/*.json` ou `$XDG_CONFIG_HOME/papecode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.papecode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.papecode/themes/*.json`

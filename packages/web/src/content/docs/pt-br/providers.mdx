---
title: Provedores
description: Usando qualquer provedor LLM no papecode.
---

import config from "../../../../config.mjs"
export const console = config.console

papecode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do papecode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/papecode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do papecode.
...
PapeCode Zen é uma lista de modelos fornecidos pela equipe do papecode que foram testados e verificados para funcionar bem com o papecode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o PapeCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `PapeCode Zen` e acesse [papecode.ai/zen](https://papecode.ai/zen).
...
Funciona como qualquer outro provedor no papecode e é completamente opcional.

---

## PapeCode Go

PapeCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do papecode que foram
testados e verificados para funcionar bem com o papecode.

1. Execute o comando `/connect` no TUI, selecione `PapeCode Go` e acesse [papecode.ai/zen](https://papecode.ai/zen).
...
Para usar o Amazon Bedrock com o papecode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o papecode:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `papecode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no papecode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o papecode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do papecode.
...
armazenar o token no armazenamento de autenticação do papecode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
papecode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o papecode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `papecode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [papecode-gitlab-auth](https://www.npmjs.com/package/papecode-gitlab-auth).

##### Configuração

Personalize através do `papecode.json`:
...
Para usar sua assinatura do GitHub Copilot com o papecode:
...
Para usar o Google Vertex AI com o papecode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o papecode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do papecode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [papecode-helicone-session](https://github.com/H2Shami/papecode-helicone-session) para registrar automaticamente cada conversa do papecode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do papecode listada como uma sessão separada.
...
Você pode configurar o papecode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o papecode para usar modelos locais através do LM Studio.
...
Você pode configurar o papecode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o papecode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/papecode) para detalhes.
:::
...
Para usar o Ollama Cloud com o papecode:
...
3. Copie a chave da API para uso no papecode.
...
6. **Importante**: Antes de usar modelos em nuvem no papecode, você deve puxar as informações do modelo localmente:
...
PapeCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do papecode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>PapeCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **PapeCode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do papecode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do papecode.
...
5. Você também pode personalizá-los através da sua configuração do papecode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o papecode:
...
Você também pode personalizar modelos através da sua configuração do papecode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o papecode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no papecode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `papecode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o papecode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `papecode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do papecode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do papecode.

---
title: Servidor
description: Interaja com o servidor papecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `papecode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente papecode pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `papecode.local` |
...
Defina `PAPECODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `papecode`, ou defina `PAPECODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `papecode serve` quanto ao `papecode web`.

```bash
PAPECODE_SERVER_PASSWORD=your-password papecode serve
```
...
Quando você executa `papecode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor papecode para interagir com o papecode programaticamente.
:::

Essa arquitetura permite que o papecode suporte múltiplos clientes e permite que você interaja com o papecode programaticamente.

Você pode executar `papecode serve` para iniciar um servidor autônomo. Se você tiver o TUI do papecode em execução, `papecode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do papecode [IDE](/docs/ide).
...
O servidor papecode expõe as seguintes APIs.

...
import { createPapecode, type ToolPart } from "@papecode-ai/sdk"
...
console.log("🚀 Starting papecode server...")
const papecode = await createPapecode({
  port: 0,
})
console.log("✅ Papecode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await papecode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new papecode session...")
    const { client, server } = papecode
    ...
    console.log("✅ Created papecode session:", createResult.data.id)
  ...
  console.log("📝 Sending to papecode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Papecode response:", JSON.stringify(result, null, 2))

...
const deepLinkEvent = "papecode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__PAPECODE__ ??= {}
  const pending = window.__PAPECODE__.deepLinks ?? []
  window.__PAPECODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__PAPECODE__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__PAPECODE__?.wsl) return result
  ...
  return {
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://papecode.ai/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__PAPECODE__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("papecode.global.dat").getItem("language")

...
const CLI_INSTALL_DIR: &str = ".papecode/bin";
const CLI_BINARY_NAME: &str = "papecode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("papecode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "PAPECODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "PAPECODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("PAPECODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.papecode/bin/papecode\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://papecode.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "PAPECODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "PAPECODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "PAPECODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "PAPECODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "PAPECODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "PAPECODE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("PAPECODE_SERVER_USERNAME", "papecode".to_string()),
        ("PAPECODE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn papecode");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("PAPECODE_CLIENT"), Some(&"desktop".to_string()));
    }

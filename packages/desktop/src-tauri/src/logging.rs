...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("papecode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("papecode_lib=debug,papecode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("papecode_lib=info,papecode_desktop=info,sidecar=info")
        }
    });

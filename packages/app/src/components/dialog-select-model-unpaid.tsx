...
import { Tag } from "@papecode-ai/ui/tag"
            ...
            <Tooltip
              ...
              value={
                <ModelTooltip
                  model={item}
                  latest={item.latest}
                  free={item.provider.id === "papecode" && (!item.cost || item.cost.input === 0)}
                />
              }
            >
              {node}
              ...
              >
                {(i) => (
                  <div class="w-full flex items-center gap-x-3">
                    ...
                    <span>{i.name}</span>
                    <Show when={i.id === "papecode"}>
                      <div class="text-14-regular text-text-weak">{language.t("dialog.provider.papecode.tagline")}</div>
                    </Show>
                    <Show when={i.id === "papecode"}>
                      <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                    </Show>
                    <Show when={i.id === "papecode-go"}>
                      <>
                        <div class="text-14-regular text-text-weak">
                          {language.t("dialog.provider.papecodeGo.tagline")}
                        </div>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </>
                    </Show>

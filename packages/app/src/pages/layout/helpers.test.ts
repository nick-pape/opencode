...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("papecode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("papecode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("papecode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("papecode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("papecode://open-project")).toBeUndefined()
    expect(parseDeepLink("papecode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "papecode://open-project?directory=/a",
      "papecode://other?directory=/b",
      "papecode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("papecode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("papecode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("papecode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("papecode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "papecode://new-session?directory=/a",
      "papecode://open-project?directory=/b",
      "papecode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __PAPECODE__: {
        deepLinks: ["papecode://open-project?directory=/a"],
      },
    } as unknown as Window & { __PAPECODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["papecode://open-project?directory=/a"])

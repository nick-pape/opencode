      ...
      retry(() =>
        input.globalSDK.project.list().then((x) => {
          const projects = (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("papecode-test"))
            .slice()
...
export const loadAgentsQuery = (
  ...
  sdk?: PapecodeClient,
  transform?: (x: Awaited<ReturnType<PapecodeClient["app"]["agents"]>>) => void,
) =>
  queryOptions<null>({
...
export const loadPathQuery = (
  directory: string | null,
  sdk?: PapecodeClient,
  transform?: (x: Awaited<ReturnType<PapecodeClient["path"]["get"]>>) => void,
) =>
  queryOptions<Path>({

...
const model = "papecode/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/papecode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with papecode...`)
  ...
  try {
    await $`papecode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  papecode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with papecode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with papecode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/papecode`.",

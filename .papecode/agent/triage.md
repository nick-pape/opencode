...
- `papecode web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `papecode web` view. **Do not** add for terminal, TUI, or general papecode issues.
...
**Only** add if the issue mentions "zen" or "papecode zen" or "papecode black".

If the issue doesn't have "zen" or "papecode black" in it then don't add zen label

#### core

Use for core server issues in `packages/papecode/`, excluding `packages/papecode/src/cli/cmd/tui/`.
...
TUI (`packages/papecode/src/cli/cmd/tui/...`):
...
Core (`packages/papecode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for papecode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
